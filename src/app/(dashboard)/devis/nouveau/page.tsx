...
export default function NouveauDevisPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setDateValidite(validite.toISOString().split("T")[0]);
    };
    load();
  }, []);
  ...
  const handleSave = async () => {
    ...
    const { data: ent } = await supabase.from("entreprise").select("prefixe_devis").single();
    const annee = new Date().getFullYear();
    const { count } = await supabase.from("devis").select("id", { count: "exact", head: true });
    const numero = `${ent?.prefixe_devis ?? "DEV"}-${annee}-${String((count ?? 0) + 1).padStart(4, "0")}`;
